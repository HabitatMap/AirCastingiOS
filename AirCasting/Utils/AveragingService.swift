...
final class AveragingService: NSObject, ObservableObject {
    ...
    private func scheduleAveraging(session: SessionEntity) {
        ...
        let fromSessionStartToFirstThreshold = (startTime.timeIntervalSince(DateBuilder.getDate()) + Double(TimeThreshold.firstThreshold.rawValue) + Double(1))
    ...
    private func averagingWindowFor(startTime: Date?) -> AveragingWindow? {
        ...
        let sessionDuration = abs(startTime.timeIntervalSince(DateBuilder.getDate()))
